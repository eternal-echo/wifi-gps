...
#include "Kalman.h"
#include "math.h"
// see the blog: http://blog.tkjelectronics.dk/2012/09/a-practical-approach-to-kalman-filter-and-how-to-implement-it
...
private:
    ...
    void getRollPitchFromAccel(float &roll, float &pitch);


    I2C_HandleTypeDef* hi2c_;
    uint8_t devAddr_; // device address
    float gyroRangeScale_;// = 131.0f / 32768.0f;
    float accRangeScale_;// = 16384.0f / 32768.0f;
    uint8_t buffer_[14];
    Kalman kalmanX_, kalmanY_;
    double kalXAngle_, kalYAngle_;
    uint32_t lastProcessed_;
    float roll_, pitch_;
    float gyroXRate_, gyroYRate_;// Convert to deg/s
