...
{
    // init wifi usart
	init(); // 确认串口和esp8266的连接
	join_wifi(ssid, password); // 连热点
	connect_host(host, port); // 连服务器
}

void WifiUsart::init(UART_HandleTypeDef *husart, const char* ssid, const char* password, const char* host, uint32_t port)
{
	Usart::init(husart); // 确认串口
	init(); // 确认串口的连接
	init(ssid,  password,  host,  port); // 驱动esp8266联网
}
