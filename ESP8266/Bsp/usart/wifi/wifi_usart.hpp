...
{
friend void Wifi_UART_RxCpltCallback(UART_HandleTypeDef *UartHandle);
/* enum and struct -----------------------------------------------------------*/
public:
    /* enum */
...
public:
    // 默认的空初始化，定义全局变量时使用
    WifiUsart() : Usart(nullptr),WiFiUartHandle(husart_) {}
    // 初始化串口handle与esp8266的连接
    WifiUsart(UART_HandleTypeDef *husart) : Usart(husart),WiFiUartHandle(husart_) {}
    // 初始化串口handle和esp8266，连接至wifi和host
...
private:
    ...
    uint8_t IpAddress[15] = {0};
    uint32_t Trial = 0;
    ESP8266_Status Status = ESP8266_Status::ESP8266_OK;


    uint8_t AtCmd[MAX_AT_CMD_SIZE] = {0};
    uint8_t RxBuffer[MAX_BUFFER_SIZE] = {0};
    RingBuffer_t WiFiRxBuffer = {0};
};

extern WifiUsart wifiUsart;
